...
if ($blockerFound.Count -gt 0) {
    ...
    try {
        ...
        $popupMsg = "检测到以下第三方安全软件正在运行:`n`n  $blockerNames`n`n这些软件会阻止脚本修改系统设置（防火墙、Defender等）。`n`n建议先手动卸载或退出这些软件，然后再继续。`n`n选择: 是 = 继续运行，否 = 退出"
        ...
        if ($result -eq [System.Windows.Forms.DialogResult]::No) {
            Write-Log "用户选择退出 (第三方安全软件问题)" "Red"
            Write-Host "按 Enter 键退出..." -ForegroundColor Red
            Save-LogFile
            Read-Host
            exit
        }
        Write-Log "用户确认继续 (忽略第三方安全软件)" "Yellow"
    } catch {
