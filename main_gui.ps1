...
function Export-IssueReport {
    ...
    $reportRoot = Join-Path $logDir "问题报告_${now}"
    $zipPath = Join-Path $logDir "问题报告_${now}.zip"
    $logPathLabel.Text = "报告位置：$zipPath"

    try {
        New-Item -ItemType Directory -Path $reportRoot -Force | Out-Null
        New-Item -ItemType Directory -Path (Join-Path $reportRoot "scripts") -Force | Out-Null
        New-Item -ItemType Directory -Path (Join-Path $reportRoot "logs") -Force | Out-Null

        Append-Output "正在收集脚本和日志..." ([System.Drawing.Color]::Cyan)
        foreach ($name in @("main_gui.ps1","system_setup.ps1","full_setup.ps1","system_restore.ps1","README.txt","AGENTS.md")) {
            $src = Join-Path $scriptDir $name
            if (Test-Path $src) { Copy-Item -Path $src -Destination (Join-Path $reportRoot "scripts") -Force }
        }
        Get-ChildItem -Path "$logDir\*.log" -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 20 |
            Copy-Item -Destination (Join-Path $reportRoot "logs") -Force -ErrorAction SilentlyContinue

        Append-Output "正在收集电脑信息..." ([System.Drawing.Color]::Cyan)
        $infoPath = Join-Path $reportRoot "电脑信息.txt"
        "问题报告生成时间: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-File -FilePath $infoPath -Encoding UTF8
        "工具目录: $scriptDir" | Out-File -FilePath $infoPath -Encoding UTF8 -Append
        "当前用户: $env:USERNAME" | Out-File -FilePath $infoPath -Encoding UTF8 -Append
        "电脑名: $env:COMPUTERNAME" | Out-File -FilePath $infoPath -Encoding UTF8 -Append
        "PowerShell: $($PSVersionTable.PSVersion)" | Out-File -FilePath $infoPath -Encoding UTF8 -Append
        try { Get-CimInstance Win32_OperatingSystem | Select-Object Caption, Version, BuildNumber, OSArchitecture, LastBootUpTime | Format-List | Out-File -FilePath $infoPath -Encoding UTF8 -Append } catch {}
        try { Get-CimInstance Win32_ComputerSystem | Select-Object Manufacturer, Model, SystemType, TotalPhysicalMemory | Format-List | Out-File -FilePath $infoPath -Encoding UTF8 -Append } catch {}
        try { Get-CimInstance Win32_BIOS | Select-Object Manufacturer, SMBIOSBIOSVersion, SerialNumber | Format-List | Out-File -FilePath $infoPath -Encoding UTF8 -Append } catch {}

        Append-Output "正在收集服务和软件清单..." ([System.Drawing.Color]::Cyan)
        $servicesPath = Join-Path $reportRoot "关键服务状态.txt"
        foreach ($svc in @("wuauserv","UsoSvc","BITS","WaaSMedicSvc","WinDefend","mpssvc","SecurityHealthService","HipsDaemon")) {
            Get-Service -Name $svc -ErrorAction SilentlyContinue | Select-Object Name, DisplayName, Status, StartType | Format-List | Out-File -FilePath $servicesPath -Encoding UTF8 -Append
        }

        $installedPath = Join-Path $reportRoot "已安装软件列表.txt"
        $uninstallPaths = @(
        ...
        foreach ($up in $uninstallPaths) {
            Get-ItemProperty -Path $up -ErrorAction SilentlyContinue |
                ...
                Out-File -FilePath $installedPath -Encoding UTF8 -Append
        }

        $softwareListPath = Join-Path $reportRoot "software文件夹清单.txt"
        $softwareDir = Join-Path $scriptDir "software"
        if (Test-Path $softwareDir) {
            Get-ChildItem -Path $softwareDir -Recurse -ErrorAction SilentlyContinue |
                Select-Object FullName, Length, LastWriteTime |
                Format-Table -AutoSize |
                Out-File -FilePath $softwareListPath -Encoding UTF8
        } else {
            "未找到 software 文件夹" | Out-File -FilePath $softwareListPath -Encoding UTF8
        }

        Append-Output "正在收集最近系统错误..." ([System.Drawing.Color]::Cyan)
        $eventsPath = Join-Path $reportRoot "最近系统错误.txt"
        foreach ($logName in @("System","Application")) {
            "===== $logName =====" | Out-File -FilePath $eventsPath -Encoding UTF8 -Append
            try {
                Get-WinEvent -FilterHashtable @{LogName=$logName; Level=1,2; StartTime=(Get-Date).AddDays(-3)} -MaxEvents 50 -ErrorAction SilentlyContinue |
                    Select-Object TimeCreated, ProviderName, Id, LevelDisplayName, Message |
                    Format-List |
                    Out-File -FilePath $eventsPath -Encoding UTF8 -Append
            } catch {
                "读取 $logName 失败: $_" | Out-File -FilePath $eventsPath -Encoding UTF8 -Append
            }
        }

        Append-Output "正在压缩问题报告..." ([System.Drawing.Color]::Cyan)
        ...
        Append-Output "[OK] 问题报告已生成: $zipPath" ([System.Drawing.Color]::FromArgb(80, 220, 80))
        [System.Windows.Forms.MessageBox]::Show("问题报告已生成：`n`n$zipPath`n`n把这个 zip 发给 AI 分析即可。", "导出完成 --龙信硬件组", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information) | Out-Null
